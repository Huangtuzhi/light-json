...
static int light_parse_string_raw(light_context* c, char** str, size_t* len) {
	//TODO
	return 0;
}

static int light_parse_string2(light_context* c, light_value* v) {
	int ret;
	char* s;
	size_t len;
	if (( ret = light_parse_string_raw(c, &s, &len)) == LIGHT_PARSE_OK)
		light_set_string(v, s, len);
	return ret;
}
...
static int light_parse_object(light_context* c, light_value* v) {
	...
	for (;;) {
		...
		// parse value
		if ((ret = light_parse_value(c, &m.value)) != LIGHT_PARSE_OK)
			break;
		memcpy(light_context_push(c, sizeof(light_member)), &m, sizeof(light_member));
		size++;
		m.key = NULL;
	}
	return ret;
