...
static void test_parse_array() {
	...
	v.type = LIGHT_NULL;
	// bug? cannot parse
	//EXPECT_EQ_INT(LIGHT_PARSE_OK, light_parse(&v, "[null,false,true,[0,1, 2], 12, \"abc\" ]"));
	EXPECT_EQ_INT(LIGHT_PARSE_OK, light_parse(&v, "[null,false,true,[0,1, 2], 12]"));
	EXPECT_EQ_INT(LIGHT_ARRAY, light_get_type(&v));
	//EXPECT_EQ_STRING("\xC2\xA2", light_get_string(&v), light_get_string_length(&v));
}
